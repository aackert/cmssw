...
struct stPlots {
   ...
   float        Tree_phi;
    ...
    float        Tree_DYQ6MVA;
    //////////////////////////
    //KENJI, make a 2-D hist for MVA for each type (array should work) to take over for the Mass histos
    //one of the following for each mapping
    //MVA
    //MVA_SystP
    //MVA_SystI  <- this is dE/dx not Ias
    //might need to add one for the HIP background
...
void stPlots_Init(TFile* HistoFile, stPlots& st, std::string BaseName, unsigned int NCuts, bool SkipSelectionPlot=false, bool isSignal=true, unsigned int NCuts_Flip=0)
{
  ...
  st.NCuts = NCuts;
   ...
   Name = "HSCPE_SystHDown";    st.HSCPE_SystHDown  = new TH1F(Name.c_str(), Name.c_str(),  NCuts, 0,  NCuts);    st.HSCPE_SystHDown    ->Sumw2();
    ...
    //KENJI //////////////// Need to add histos for TOF
...
void stPlots_FillTree(stPlots* st, unsigned int Run, unsigned int Event, unsigned int Hscp, double Pt, double I, double TOF, double Mass, double dZ, double dXY, double dR, double eta, double phi, double dedx, double P, int charge, double Gluino_mva, double Stop_mva, double GMSBStau_mva, double PPStau_mva, double DYQ3_mva, double DYQ6_mva, int MaxEntry=20000){
...
//   c1->SetLogy(true);
   ...
   delete c1;
    ...
    //KENJI --> Add more plots as you go along  //////////////////////////////////////////////////////
    if( TypeMode > 5){
        ...
        vector<string> mvalegnames = {"Gluino MVA","Stop MVA","GMStau MVA", "PPStau MVA", "DYQ1 MVA", "DYQ2 MVA"};
        
        
        c1 = new TCanvas("c1","c1,",600,600);          legend.clear();
        c1->SetLogy(true);
        for( int ihist = 0; ihist < Nmva; ihist++){
            if(ReaderExists[ihist]){
                Histos1D[ihist] = (TH1*)st.MVA[0] -> Clone(); legend.push_back(mvalegnames[ihist].c_str()); Histos1D[ihist]->Rebin(1);
            }
        }
        DrawSuperposedHistos((TH1**)Histos, legend, "HIST",  "MVA", "Number of tracks", 0, 0, 0, 0, false, false, true, true);
        DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
        SaveCanvas(c1,SavePath,"GluinoMVAdist", false);
        delete c1;
    }
    //////////////////////////////////////////////////////
}
