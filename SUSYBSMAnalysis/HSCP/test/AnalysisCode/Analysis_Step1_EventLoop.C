...
void Analysis_FillControlAndPredictionHist(const susybsm::HSCParticle& hscp, const reco::DeDxData* dedxSObj, const reco::DeDxData* dedxMObj, const reco::MuonTimeExtra* tof, stPlots* st, double mvavalues[]){ //KENJI
    reco::TrackRef   track;
    if(TypeMode!=3) track = hscp.trackRef();
    else {
        reco::MuonRef muon = hscp.muonRef();
    ...
    for(unsigned int CutIndex=0;CutIndex<CutPt.size();CutIndex++){
        ...
        if(       PassTOFCut &&  PassPtCut &&  PassICut){   //Region D
            ...
            if(TypeMode<2|| TypeMode == 6)st->Pred_EtaP  ->Fill(CutIndex,track->eta(), track->p(),     Event_Weight);
            ...
            if(TypeMode<2|| TypeMode == 6)st->Pred_I  ->Fill(CutIndex,Ih, Event_Weight);
            if(TypeMode<2|| TypeMode == 6)st->Pred_EtaS->Fill(CutIndex,track->eta(),         Event_Weight);
            //Pred_TOF->Fill(CutIndex,MuonTOF,         Event_Weight);
            st->AS_Eta_RegionB->Fill(CutIndex,track->eta());
        }else if( PassTOFCut && !PassPtCut && !PassICut){   //Region A
            st->H_A     ->Fill(CutIndex,                 Event_Weight);
            if(TypeMode==2|| TypeMode == 7)st->Pred_TOF->Fill(CutIndex,MuonTOF,         Event_Weight);
            if(TypeMode<2|| TypeMode == 6 )st->Pred_EtaB->Fill(CutIndex,track->eta(),         Event_Weight);
            if(TypeMode==2|| TypeMode == 7)st->Pred_EtaS2->Fill(CutIndex,track->eta(),        Event_Weight);
            ...
            if(TypeMode==2|| TypeMode == 7)st->Pred_EtaP  ->Fill(CutIndex,track->eta(),track->p(),     Event_Weight);
            ...
            if(TypeMode==2|| TypeMode == 7)st->Pred_I  ->Fill(CutIndex,Ih, Event_Weight);
            if(TypeMode==2|| TypeMode == 7)st->Pred_EtaS->Fill(CutIndex,track->eta(),         Event_Weight);
            st->AS_Eta_RegionF->Fill(CutIndex,track->eta());
        }else if(!PassTOFCut && !PassPtCut && !PassICut){   //Region E
            st->H_E     ->Fill(CutIndex,                 Event_Weight);
            if(TypeMode==2|| TypeMode == 7)st->Pred_EtaB->Fill(CutIndex,track->eta(),         Event_Weight);
    ...
    for(unsigned int CutIndex=0;CutIndex<CutPt_Flip.size();CutIndex++){
        ...
        if(       PassTOFCut &&  PassPtCut &&  PassICut){   //Region D
            ...
            if(TypeMode<2|| TypeMode == 6)st->Pred_EtaP_Flip->Fill(CutIndex,track->eta(), track->p(),     Event_Weight);
            ...
            if(TypeMode<2|| TypeMode == 6)st->Pred_I_Flip->Fill(CutIndex,Ih, Event_Weight);
            if(TypeMode<2|| TypeMode == 6)st->Pred_EtaS_Flip->Fill(CutIndex,track->eta(),         Event_Weight);
            //Pred_TOF_Flip->Fill(CutIndex,MuonTOF,         Event_Weight);
        }else if( PassTOFCut && !PassPtCut && !PassICut){   //Region A
            st->H_A_Flip->Fill(CutIndex,                 Event_Weight);
            if(TypeMode==2|| TypeMode == 7)st->Pred_TOF_Flip->Fill(CutIndex,MuonTOF,         Event_Weight);
            if(TypeMode<2|| TypeMode == 6)st->Pred_EtaB_Flip->Fill(CutIndex,track->eta(),         Event_Weight);
            if(TypeMode==2|| TypeMode == 7)st->Pred_EtaS2_Flip->Fill(CutIndex,track->eta(),        Event_Weight);
        }else if(!PassTOFCut &&  PassPtCut &&  PassICut){   //Region H
            ...
            if(TypeMode==2|| TypeMode == 7)st->Pred_EtaP_Flip->Fill(CutIndex,track->eta(),track->p(),     Event_Weight);
        }else if(!PassTOFCut && !PassPtCut &&  PassICut){   //Region F
            st->H_F_Flip->Fill(CutIndex,                 Event_Weight);
            if(bin>-1 && bin<MaxPredBins) st->H_F_Binned_Flip[bin]->Fill(CutIndex,                Event_Weight);
            if(TypeMode==2|| TypeMode == 7)st->Pred_I_Flip->Fill(CutIndex,Ih, Event_Weight);
            if(TypeMode==2|| TypeMode == 7)st->Pred_EtaS_Flip->Fill(CutIndex,track->eta(),         Event_Weight);
        }else if(!PassTOFCut && !PassPtCut && !PassICut){   //Region E
            st->H_E_Flip->Fill(CutIndex,                 Event_Weight);
            if(TypeMode==2|| TypeMode == 7)st->Pred_EtaB_Flip->Fill(CutIndex,track->eta(),         Event_Weight);
        }
    }
}
...
void Analysis_Step1_EventLoop(char* SavePath)
{
    ...
    for( int iReader = 0; iReader < 6; iReader++){
        ...
        if(TypeMode == 7){
            ReaderExists[iReader] = MVAWeightexists(MVAtype7class[iReader]);
        }
    }
    ...
    ReadMLPGluino mvaGluino(names);
    ReadMLPStop mvaStop(names);
    ReadMLPGMSBStau mvaGMSBStau(names);
    ReadMLPPPStau mvaPPStau(names);
    ...
    for(unsigned int s=0;s<samples.size();s++){
        ...
        for (int period=0; period<(samples[s].Type>=2?RunningPeriods:1); period++){
            ...
            for(Long64_t ientry=0;ientry<ev.size();ientry++){
                ...
                for(unsigned int c=0;c<hscpColl.size();c++){
                            ...
                            for(unsigned int CutIndex=0;CutIndex<CutPt.size();CutIndex++){
                                if(PassSelection(hscp,  dedxSObj, dedxMObj, tof, ev, CutIndex, NULL, false, -1,   PRescale, 0, 0)){
                                    ...
                                    if(TypeMode == 6){
                                        ...
                                        mvainput[1] = dedxMObj->dEdx();
                                        mvainput[2] = track->eta();
                                        for(int ihist = 0; ihist < 6; ihist++){
                                            if(ReaderExists[ihist]){
                                                if(ihist == 0){mvaValsyst[ihist] = mvaGluino.GetMvaValue(mvainput);}
                                                if(ihist == 1){mvaValsyst[ihist] = mvaStop.GetMvaValue(mvainput);}
                                                if(ihist == 2){mvaValsyst[ihist] = mvaGMSBStau.GetMvaValue(mvainput); }
                                                if(ihist == 3){mvaValsyst[ihist] = mvaPPStau.GetMvaValue(mvainput); }
                                                
                                                SamplePlots->MVA_SystP[ihist]->Fill(CutIndex, mvaValsyst[ihist],Event_Weight);
                                            }
                                        }
                                    }
                                    if(tof){
                                        ...
                                        mvainput[1] = dedxMObj->dEdx();
                                        mvainput[2] = track->eta();
                                        mvainput[3] = tof ? tof->inverseBeta() : -1;
                                        for(int ihist = 0; ihist < 5; ihist++){
                                            //KENJI ADD TOF MVA hists
                                        }
                                    }
                                    /////////////////////////////////////
                                    if(tof){
                            ...
                            for(unsigned int CutIndex=0;CutIndex<CutPt.size();CutIndex++){
                                if(PassSelection(hscp,  dedxSObj, dedxMObj, tof, ev, CutIndex, NULL, false, -1,   0, IRescale, 0)){
                                    ...
                                    if(TypeMode == 6){
                                        mvainput.resize(3);
                                        mvainput[0] = track->p();
                                        mvainput[1] = dedxMObj->dEdx()*MRescale;
                                        mvainput[2] = track->eta();
                                        ...
                                        for(int ihist = 0; ihist < 6; ihist++){
                                            if(ReaderExists[ihist]){
                                                ...
                                                if(ihist == 2){mvaValsyst[ihist] = mvaGMSBStau.GetMvaValue(mvainput); }
                                                if(ihist == 3){mvaValsyst[ihist] = mvaPPStau.GetMvaValue(mvainput); }
                                                SamplePlots->MVA_SystI[ihist]->Fill(CutIndex, mvaValsyst[ihist],Event_Weight);
                                            }
                                        }
                                    }
                                    if(tof){
                    ...
                    //fill the ABCD histograms and a few other control plots  /// CHECK THIS //// KENJI
                    ////KENJI /////////////////////////////////////
                    ...
                    for( int ihist = 0; ihist < 6; ihist++){
                        if( ReaderExists[ihist]){
                            if(ihist == 0){mva_value[ihist] = mvaGluino.GetMvaValue(mvainput);}
                            if(ihist == 1){mva_value[ihist] = mvaStop.GetMvaValue(mvainput);}
                            if(ihist == 2){mva_value[ihist] = mvaGMSBStau.GetMvaValue(mvainput); }
                            if(ihist == 3){mva_value[ihist] = mvaPPStau.GetMvaValue(mvainput); }
                            //if(ihist == 4){mva_value[ihist] == mvaDYQ3.GetMvaValue(mvainput); }
                            //if(ihist == 5){mva_value[ihist] == mvaDYQ6.GetMvaValue(mvainput); }
                        }
                    ...
                    /////////////////////////////////////
                    //KENJI
                    if(isData && TypeMode < 6)Analysis_FillControlAndPredictionHist(hscp, dedxSObj, dedxMObj, tof, SamplePlots);
                    if(isData && TypeMode > 5)Analysis_FillControlAndPredictionHist(hscp, dedxSObj, dedxMObj, tof, SamplePlots, mva_value); //KENJI
                    if(isMC && TypeMode < 6) Analysis_FillControlAndPredictionHist(hscp, dedxSObj, dedxMObj, tof, MCTrPlots);
                    if(isMC && TypeMode > 5) Analysis_FillControlAndPredictionHist(hscp, dedxSObj, dedxMObj, tof, MCTrPlots, mva_value); //KENJI
                    if(TypeMode==5 && isCosmicSB)continue;
                    ...
                    for(unsigned int CutIndex=0;CutIndex<CutPt.size();CutIndex++){
                        ...
                        for( int ihist = 0; ihist < 6; ihist++){
                            if(ReaderExists[ihist]){
                                ...
                                SamplePlots -> MVA[ihist]->Fill(CutIndex,mva_value[ihist],Event_Weight);
                                if(mva_value[ihist]>MaxMVA[CutIndex][ihist]){MaxMVA[CutIndex][ihist]=mva_value[ihist];}
                            }
                        }
                        //////////////////////////////////////
                        //Fill Mass Histograms
                    ...
                    //FILL HSCP Candidate Tree ///KENJI
                     if(PassNonTrivialSelection) stPlots_FillTree(SamplePlots, ev.eventAuxiliary().run(),ev.eventAuxiliary().event(), c, track->pt(), dedxSObj ? dedxSObj->dEdx() : -1, tof ? tof->inverseBeta() : -1, Mass, TreeDZ, TreeDXY, OpenAngle, track->eta(), track->phi(), dedxMObj->dEdx(), track->p(), track->charge(), mva_value[0], mva_value[1], mva_value[2], mva_value[3], mva_value[4], mva_value[5], -1);
                }// end of Track Loop
        ...
        //KENJI might need to delete MaxMVA etc.
