...
void Analysis_Step2_BackgroundPrediction(std::string InputPattern="COMPILE")
{
    ...
    for( int iReader = 0; iReader < 4; iReader++){
        ReaderExists[iReader] = true;
        //if(TypeMode == 6){
    ...
    //ReadMLPDYQ3 mvaDYQ3(names);
    //ReadMLPDYQ6 mvaDYQ6(names);
    //and so forth
    
    //need to change the mva classes to different names ReadMLP->ReadMLPGluino
    ...
    for(unsigned int S=0; S<2; S++) {
        ...
        for(int d=0;d<list->GetEntries();d++){
            ...
            if(S==0){ //don't need to do the TOF flip for now
                for( int ihist = 0; ihist < 6; ihist++){
                    Pred_MVA[ihist] = (TH2D*)GetObjectFromPath(directory, (MVAhistname[ihist]+Suffix).c_str())->Clone(("Pred_"+MVAhistname[ihist]).c_str());
                }
            }
            //////////////
            ...
            for(unsigned int CutIndex=0;CutIndex<(unsigned int)HCuts_Pt->GetXaxis()->GetNbins();CutIndex++){
                ...
                string MVAhistname[6] = {"Pred_Prof_GluinoMVA","Pred_Prof_StopMVA","Pred_Prof_GMSBStauMVA","Pred_Prof_PPStauMVA","Pred_Prof_DYQ3MVA","Pred_Prof_DYQ6MVA"};
                for( int ihist = 0; ihist < 6; ihist++){
                    Pred_Prof_MVA[ihist] = new TH2D(MVAhistname[ihist].c_str(),MVAhistname[ihist].c_str(), MVANBins, 0, MVAHistoUpperBound, NPseudoExp, 0, NPseudoExp);
                }
                ...
                for(unsigned int pe=0;pe<NPseudoExp;pe++){
                    ...
                    if(TypeMode < 6){
                        for( int ihist = 0; ihist < 6; ihist++){
                            delete tmpH_MVA[ihist]; //tmpH_MVA not needed if TypeMode < 6
                        }
                    }
                    //////////////////
                    ...
                    TH1D* Pred_Eta_ProjPE = Pred_EtaPWeighted_PE->ProjectionX("Pred_Eta_ProjPE");
                    ...
                    for( int nMVA = 0; nMVA < N; nMVA++){
                        ...
                        for(int x=0;x<Pred_P_ProjPE->GetNbinsX()+1;x++){    if(Pred_P_ProjPE->GetBinContent(x)<=0.0){continue;}  const double& p = Pred_P_ProjPE->GetBinCenter(x);
                            for(int y=0;y<Pred_I_ProjPE->GetNbinsX()+1;y++){    if(Pred_I_ProjPE->GetBinContent(y)<=0.0){continue;}  const double& i = Pred_I_ProjPE->GetBinCenter(y);
                                ...
                                //KENJI ADD The MVA calc in here/////////////////////////
                                if(TypeMode > 5){
                                    for( int z=0; z<Pred_Eta_ProjPE->GetNbinsX()+1;z++){
                                        if(Pred_Eta_ProjPE->GetBinContent(z)<=0.0){continue;}
                                        double eta = Pred_Eta_ProjPE->GetBinCenter(z);
                                        
                                        Proba = Pred_P_ProjPE->GetBinContent(x) * Pred_I_ProjPE->GetBinContent(y) * Pred_Eta_ProjPE->GetBinContent(z);
                                        if(Proba<=0 || isnan((float)Proba))continue;
                                        
                                        vector<double> mvainput(3);
                                        mvainput[0] = p;
                                        mvainput[1] = i;
                                        mvainput[2] = eta;
                                        // test only 1 first
                                        
                                        if( nMVA == 0 && ReaderExists[nMVA]){
                                            mvavalues[nMVA] = mvaGluino.GetMvaValue(mvainput);
                                        }
                                        else if( nMVA == 1 && ReaderExists[nMVA]){
                                            mvavalues[nMVA] = mvaStop.GetMvaValue(mvainput);
                                        }
                                        else if( nMVA == 2 && ReaderExists[nMVA]){
                                            mvavalues[nMVA] = mvaGMSBStau.GetMvaValue(mvainput);
                                        }
                                        else if( nMVA == 3 && ReaderExists[nMVA]){
                                            mvavalues[nMVA] = mvaPPStau.GetMvaValue(mvainput);
                                        }
                                        else if( nMVA == 4 && ReaderExists[nMVA]){
                                            //mvavalues[nMVA] = mvaDYQ3.GetMvaValue(mvainput);
                                        }
                                        else if( nMVA == 5 && ReaderExists[nMVA]){
                                            //mvavalues[nMVA] = mvaDYQ6.GetMvaValue(mvainput);
                                        }
                    ...
                    if(TypeMode > 5 ){
                        for(int ihist = 0; ihist<6; ihist++){
                            for(int x=0; x<tmpH_MVA[ihist]->GetNbinsX()+1;x++){
                                Pred_Prof_MVA[ihist] -> SetBinContent(x, pe, tmpH_MVA[ihist]->GetBinContent(x)*PE_P);
                                if(isnan((float)(tmpH_MVA[ihist]    ->GetBinContent(x) * PE_P))){printf("%f x %f\n",tmpH_MVA[ihist]    ->GetBinContent(x),PE_P); fflush(stdout);exit(0);}
                            }
                ...
                if(TypeMode > 5){
                    ...
                    for( int ihist = 0; ihist<6; ihist++){
                        for( int x=0; x<Pred_MVA[ihist]->GetNbinsY()+1;x++){
                            double Mean = 0, MeanTOF=0, MeanComb=0;
                            for( unsigned int pe=0;pe<NPseudoExp;pe++){
                                Mean     += Pred_Prof_MVA[ihist] ->GetBinContent(x, pe);
                            }
                            Mean /= NPseudoExp;
