...
AnalysesToRun = [0,6]#,4]#,3,5]

UseRemoteSamples = True
#RemoteStorageDir = '/storage/data/cms/store/user/jozobec/LoicNTuples/15_03_25_HSCP_Run2EDMFiles/'
RemoteStorageDir = '/store/user/lpchscp/noreplica/' #KENJI
           ...
           if((vals[0].replace('"','')) in CMSSW_VERSION):
              for Type in AnalysesToRun:
                      if(UseRemoteSamples):
                              ...
                              LaunchOnCondor.Jobs_InitCmds = ['ulimit -c 0', 'export HOME=%s' % os.environ['HOME'], 'export X509_USER_PROXY=$HOME/x509_user_proxy/x509_proxy; voms-proxy-init --noregen;', 'export REMOTESTORAGEPATH='+RemoteStorageDir] #KENJI
